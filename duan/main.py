...
import pandas as pd
import json
# Import câu hỏi từ file cauhoi.py
from cauhoi import bo_cau_hoi

# Tệp JSON lưu trữ thông tin người chơi
...
def man_hinh_chao_mung():
    ...
    tk.Button(manhinh, text="Tiếp tục", command=man_hinh_the_loai_cau_hoi, font=("Arial", 16), bg="#4CAF50", fg="white").pack(pady=40)
...
def man_hinh_the_loai_cau_hoi():
    ...
    for category in questions.keys():
        ttk.Radiobutton(manhinh, text=category, variable=category_var, value=category, style="primary.TRadiobutton").pack(pady=15)
...
def kiem_tra_dap_an(selected_option):
    global current_question, score, player_name
    player_name = player_name_var.get()
    if selected_option == questions[selected_category][current_question]["answer"]:
        score += 1
        correct_sound.play()
        messagebox.showinfo("Đúng!", f"Chúc mừng,{player_name}  đã trả lời đúng!")
    else:
        wrong_sound.play()
